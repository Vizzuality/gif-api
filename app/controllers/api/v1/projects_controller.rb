module Api::V1
  class ProjectsController < ApiController
    ...
    def index
      respond_to do |format|
        format.json do
          ...
          if Rails.cache.exist?(key_string)
            render json: Rails.cache.read(key_string)
          else
            # rubocop:disable LineLength
            Rails.cache.fetch(key_string) { render json: Project.fetch_all(filter_params), each_serializer: ProjectSerializer }
            # rubocop:enable LineLength
          end
        end
        format.csv do
    ...
    private
      def filter_params
        params.permit(:q, :summary, :order, :direction, :name,
                      :from_cost, :to_cost, :offset, :limit,
                      :other_organization, :other_donor,
                      co_benefits:[], primary_benefits:[], status:[],
                      scales:[], organizations:[], donors:[], countries:[],
                      regions:[], hazard_types:[], intervention_types:[],
